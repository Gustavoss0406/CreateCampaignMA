...
def extract_fb_error(resp: requests.Response) -> str:
    try:
        ...
        return e.get("error_user_msg") or e.get("message") or resp.text
    except:
        return resp.text or "Unknown Facebook API error"

def rollback_campaign(campaign_id: str, token: str):
    try:
        requests.delete(
            f"https://graph.facebook.com/{FB_API_VERSION}/{campaign_id}",
            params={"access_token": token}
        )
        logger.info(f"[rollback] Campaign {campaign_id} deleted")
    except:
        logger.exception("[rollback] Failed to delete campaign")

def upload_video_to_fb(account_id: str, token: str, video_url: str) -> str:
    resp = requests.post(
        f"https://graph.facebook.com/{FB_API_VERSION}/act_{account_id}/advideos",
        data={"file_url": video_url, "access_token": token}
    )
    logger.debug(f"[video upload] {resp.status_code}: {resp.text}")
    if resp.status_code != 200:
        raise Exception(f"Video upload error: {extract_fb_error(resp)}")
    vid = resp.json().get("id")
    if not vid:
        raise Exception("Facebook did not return a video_id")
    return vid
...
def fetch_video_thumbnail(video_id: str, token: str) -> str:
    ...
    for _ in range(5):
        ...
        if data and data[0].get("uri"):
            return data[0]["uri"]
        time.sleep(2)
    raise Exception("Could not retrieve video thumbnail")
...
def get_page_id(token: str) -> str:
    ...
    if resp.status_code != 200:
        raise HTTPException(status_code=resp.status_code, detail="Error fetching pages")
    arr = resp.json().get("data", [])
    if not arr:
        raise HTTPException(status_code=533, detail="No Facebook Page available")
    return arr[0]["id"]
...
def check_account_balance(account_id: str, token: str, required_cents: int):
    ...
    if resp.status_code != 200:
        raise HTTPException(status_code=resp.status_code, detail="Error checking balance")
    js = resp.json()
...
class CampaignRequest(BaseModel):
    ...
    target_sex:     str  # "male", "female", or "all"
    target_age:     int
    image:          str = ""
    carrossel:      List[str] = []
    video:          str = Field(default="", alias="video")
    lead_form_id:   Optional[str] = ""  # new: ID do formulário de lead
...
async def create_campaign(req: Request):
    ...
    logger.info(f"Creating campaign: {data.campaign_name}")
    ...
    if daily < 576:
        ...
        raise HTTPException(status_code=400, detail="Daily budget must be ≥ $5.76")
    if (end_dt - start_dt) < timedelta(hours=24):
        rollback_campaign(camp_id, data.token)
        raise HTTPException(status_code=400, detail="Campaign duration must be ≥ 24h")

    # 4) Determine optimization and billing, prepare adset payload
    page_id = get_page_id(data.token)
    if data.objective == "OUTCOME_AWARENESS":
        opt_goal = billing_event = "IMPRESSIONS"
        promoted_object = {}
    elif data.objective == "OUTCOME_LEADS":
        # require form ID
        if not data.lead_form_id:
            rollback_campaign(camp_id, data.token)
            raise HTTPException(status_code=400, detail="lead_form_id is required for LEAD_GENERATION")
        opt_goal      = "LEAD_GENERATION"
        billing_event = "IMPRESSIONS"
        promoted_object = {
            "page_id":         page_id,
            "leadgen_form_id": data.lead_form_id
        }
    else:
        opt_goal = billing_event = "LINK_CLICKS"
