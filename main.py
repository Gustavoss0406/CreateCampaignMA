...
def extract_fb_error(response: requests.Response) -> str:
    try:
        err = response.json()
        return err.get("error", {}).get("error_user_msg", "Erro desconhecido")
    except:
...
async def create_campaign(request: Request):
    ...
    camp = requests.post(
        ...
        json={"name": data.campaign_name, "objective": data.objective, "status": "ACTIVE", "access_token": token}
    )
    ...
    # 6) Upload do vídeo, se fornecido como URL externo
    video_id = None
    if data.video.strip():
        upload = requests.post(
            f"https://graph.facebook.com/{fb_api}/act_{acct}/advideos",
            data={"file_url": data.video.strip(), "access_token": token}
        )
        if upload.status_code != 200:
            err = extract_fb_error(upload)
    ...
    adset = requests.post(f"https://graph.facebook.com/{fb_api}/act_{acct}/adsets", json=adset_payload)
    ...
    if video_id:
        creative_spec = {"video_data": {"video_id": video_id, "title": data.campaign_name, "message": default_msg}}
